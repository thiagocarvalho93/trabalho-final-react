...
const Produto = () => {
    ...
    const getProdutos = async () => {
      ...
      try {
        const { data } = await axios.get(
          "https://teg-store-api.herokuapp.com/tegloja/produtos"
        );
        setProdutos(data);
        if (!location.state) {
  ...
  useEffect(() => {
    ...
    const searchCategoria = async () => {
      try {
        const { data } = await axios.get(
          `https://teg-store-api.herokuapp.com/tegloja/categorias/${option}/produtos`
        );
        setProdutos(data);
        setProdutosFiltrados(data);
      } catch (e) {
  ...
  const filtraProdutos = () => {
    setProdutosFiltrados(
      produtos.filter((produto) => produto.valorUnitario < precoMax)
    );
  };
